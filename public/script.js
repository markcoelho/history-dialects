...
document.addEventListener("DOMContentLoaded", () => {
  ...
  // blur/clear effect when hovering intro paragraph
  const introP = document.querySelector(".intro p");
  const banner = document.querySelector(".banner-container");
  if (introP && banner) {
    introP.addEventListener("mouseenter", () => {
      banner.classList.add("blur");
      introP.classList.add("clear");
    });
    introP.addEventListener("mouseleave", () => {
      banner.classList.remove("blur");
      introP.classList.remove("clear");
    });

    // scroll to generator section when paragraph is clicked
  ...
  async function generateAndDisplay(event, style) {
    try {
      ...
      stopPlayback();
      resultDiv.innerHTML =
        '<div class="spinner"></div><p>Generating description...</p>';
  ...
  form.addEventListener("submit", async (e) => {
    ...
    if (!event) {
      resultDiv.innerHTML =
        '<div class="error">Please enter a historical event</div>';
      return;
    }
  ...
  async function playTTS(text) {
    /* TTS playback logic would go here */
  }
